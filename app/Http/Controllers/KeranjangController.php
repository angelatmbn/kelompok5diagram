...
class KeranjangController extends Controller
{
    ...
    public function daftarmenu()
    {
        ...
        $jmlmenuibeli = DB::table('penjualan')
    ...
    public function tambahKeranjang(Request $request)
    {
        ...
        try {
            ...
            $pelanggan = Pembeli::where('user_id', $id_user)
                            ->select(DB::raw('id'))
                            ->first();
            $id_pembeli = $pelanggan->id;
            ...
            try{
               ...
               // Cek apakah ada penjualan dengan gross_amount = 0
                ...
                $jmlbarangdibeli = DB::table('penjualan')
                            ->join('penjualan_barang', 'penjualan.id', '=', 'penjualan_barang.penjualan_id')
                            ->join('pelanggan', 'penjualan.pembeli_id', '=', 'pelanggan.id')
                            ->join('pembayaran', 'penjualan.id', '=', 'pembayaran.penjualan_id')
                            ->select(DB::raw('COUNT(DISTINCT barang_id) as total'))
                            ->where('penjualan.pembeli_id', '=', $id_pembeli) 
                            ->where(function($query) {
    ...
    public function lihatkeranjang(){
        ...
        $barang = DB::table('penjualan')
                        ...
                        ->join('pelanggan', 'penjualan.pembeli_id', '=', 'pelanggan.id')
                        ->select('penjualan.id','penjualan.no_faktur','pelanggan.nama_pembeli', 'penjualan_barang.barang_id', 'barang.nama_barang','penjualan_barang.harga_jual', 
                        ...
                        ->groupBy('penjualan.id','penjualan.no_faktur','pelanggan.nama_pembeli','penjualan_barang.barang_id', 'barang.nama_barang','penjualan_barang.harga_jual',
                                  'barang.foto','pembayaran.order_id',
                                 )
                        ->get();
